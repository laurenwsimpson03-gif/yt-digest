...
def search_topics(topics: list[str], max_per_topic: int = 5) -> list[dict]:
    ...
    since = _published_since(30)  # broader window for discovery
    seen_ids = set()
    all_videos = []
    ...
    for topic in topics:
        data = _get(
            ...
            q=topic,
            type="video",
            videoDuration="long",       # >20 min (YouTube API definition)
            order="relevance",
...
def get_transcript_snippet(video_id: str, max_chars: int = 800) -> str:
    """
    Fetch auto-generated transcript via youtube-transcript-api.
    Returns first ~800 chars of transcript text, or empty string on failure.
    """
    try:
...
def apply_filters(videos: list[dict], cfg: dict) -> list[dict]:
    ...
    f = cfg["filters"]
    min_dur = f["min_duration_minutes"]
    max_dur = f["max_duration_minutes"]
    min_views = f.get("min_view_count", 0)
    exclude = [kw.lower() for kw in f.get("exclude_keywords", [])]

    filtered = []
    for v in videos:
        if v["duration_minutes"] < min_dur:
            continue
        if v["duration_minutes"] > max_dur:
            continue
